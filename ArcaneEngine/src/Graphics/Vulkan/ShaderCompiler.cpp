...
struct SpirvHelper
{
	...
	static bool GLSLtoSPV(const EShLanguage shader_type, const char* pshader, std::vector<unsigned int>& spirv) {
...
namespace Arc::ShaderCompiler
{
	...
	bool Compile(const std::string& filePath, ShaderDesc& shaderDesc)
	{
		...
		{
			ARC_LOG_ERROR("Could not open file: " + filePath);
		}
		...
		bool success = SpirvHelper::GLSLtoSPV(shaderStage, result.c_str(), shaderDesc.SpirV);
		if (!success)
		{
			ARC_LOG_ERROR("Failed to compile shader " + filePath);
		}
		return success;
	}
