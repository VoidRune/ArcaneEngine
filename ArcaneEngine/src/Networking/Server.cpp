...
namespace Arc
{
	...
	void Server::ConnectionStatusChangedCallback(SteamNetConnectionStatusChangedCallback_t* status)
	{
		s_Instance->OnConnectionStatusChanged(status);
	}
	...
	void Server::OnConnectionStatusChanged(SteamNetConnectionStatusChangedCallback_t* status)
	{
		...
		case k_ESteamNetworkingConnectionState_ProblemDetectedLocally:
		{
			...
			if (status->m_eOldState == k_ESteamNetworkingConnectionState_Connected)
			{
				...
				// User callback
				m_ClientDisconnectedCallback(status->m_hConn);
		...
		case k_ESteamNetworkingConnectionState_Connecting:
		{
			...
			client = connectionInfo.m_szConnectionDescription;
			//client.ConnectionDesc = connectionInfo.m_szConnectionDescription;

			// User callback
			m_ClientConnectedCallback(status->m_hConn);
	...
	void Server::PollIncomingMessages()
	{
		...
		while (m_Running)
		{
			...
			{
				m_DataReceivedCallback(incomingMessage->m_conn, Buffer(incomingMessage->m_pData, incomingMessage->m_cbSize));
			}
	...
	void Server::SetClientNick(ClientID hConn, const char* nick)
	{
